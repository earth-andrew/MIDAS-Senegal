        ...
        for indexL = 1:length(bestLocations)
            ...
            if ~isempty(nextBest)
                ...
                if any(selectableLayers' & focalAspiration)
                    [currentAgent.bestPortfolios{locationIndex}, ~] = createPortfolio(currentAgent.currentAspiration, [],utilityVariables.utilityTimeConstraints, utilityVariables.utilityPrereqs, currentAgent.pAddFitElement, currentAgent.training, currentAgent.experience, currentAgent.layerFlag, utilityVariables.utilityAccessCosts, utilityVariables.utilityRestrictions, utilityVariables.utilityDuration, currentAgent.numPeriodsEvaluate, selectableLayers, [], currentAgent.wealth, backCastNum, utilityVariables.utilityAccessCodesMat, modelParameters);
                    %Adjust time of high-fidelity duration if new experience helps fulfill prereq
                else
                    %Ensure portfolio is not empty
