...
for indexA = 1:length(agentList)
   ...
   [currentAgent.currentPortfolio,currentAgent.currentAspiration,currentAgent.currentFidelity] = createPortfolio([], find(utilityVariables.utilityBaseLayers(currentAgent.matrixLocation,:,1) ~= -9999),utilityVariables.utilityTimeConstraints, utilityVariables.utilityPrereqs, currentAgent.pAddFitElement, currentAgent.training, currentAgent.experience, utilityVariables.utilityAccessCosts, utilityVariables.utilityDuration, currentAgent.numPeriodsEvaluate, selectable, utilityVariables.utilityHistory(1,:,1), currentAgent.wealth, currentAgent.pBackCast);
